...
u8 m100_refresh;
float m100_yaw;
int Qr_pos[2],Drone_pos[2],Tar_pos[2];
void Data_Receive_Anl5(u8 *data_buf,u8 num)
{
   ...
   if(*(data_buf+2)==0x01)//IMU_FRAME
  {
	  ...
	  fly_ready=		  *(data_buf+4);	
		...
		Qr_pos[0]= ((vs16)(*(data_buf+8)<<8)|*(data_buf+9));
		Qr_pos[1]= ((vs16)(*(data_buf+10)<<8)|*(data_buf+11));
		Drone_pos[0]= ((vs16)(*(data_buf+12)<<8)|*(data_buf+13));
		Drone_pos[1]= ((vs16)(*(data_buf+14)<<8)|*(data_buf+15));
		Tar_pos[0]= ((vs16)(*(data_buf+16)<<8)|*(data_buf+17));
		Tar_pos[1]= ((vs16)(*(data_buf+18)<<8)|*(data_buf+19));
...
u16 PWM_DJI[4]={0};
void Data_Receive_Anl5_1(u8 *data_buf,u8 num)
{
	vs16 rc_value_temp;
	u8 sum = 0;
	u8 i;
...
void UsartSend5(uint8_t ch)
{

	while(USART_GetFlagStatus(UART5, USART_FLAG_TXE) == RESET);
USART_SendData(UART5, ch); 
}
static void Uart5_Send_bufs(u8 *dataToSend , u8 length)
{
u16 i;
  for(i=0;i<length;i++)
     UsartSend5(dataToSend[i]);
}


void Send_IMU_FC(void)
{u8 i;	u8 sum = 0;
	u8 data_to_send[50];
	u8 _cnt=0;
	vs16 _temp;
	...
	_temp = laser_distance;//ultra_distance;
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
  _temp = Strength;//ultra_distance;
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
	...
	Uart5_Send_bufs(data_to_send, _cnt);
}  
...
{u8 i;	u8 sum = 0;//u8 SendBuff1[50];
  ...
  SendBuff1[SendBuff1_cnt++]=BYTE3(_temp);
	...
	SendBuff1[SendBuff1_cnt++]=BYTE0(_temp);
   //hight
	_temp=(vs32)(LIMIT(HEIGHT/10,0,50*100));
	SendBuff1[SendBuff1_cnt++]=BYTE3(_temp);
	SendBuff1[SendBuff1_cnt++]=BYTE2(_temp);
	SendBuff1[SendBuff1_cnt++]=BYTE1(_temp);
	SendBuff1[SendBuff1_cnt++]=BYTE0(_temp);
		...
		case 2:
			 ...
			 {state=0;
				 ...
				 Strength=buf[3]<<8|buf[2];
					 ...
					 if(Strength>10)
					 { temp1=1;//limit_mine(Roll,45);
						laser_distance=(buf[1]<<8|buf[0])*cos(temp1*0.017)*10/2;laser_check[0]=1;laser_cnt[0]=0;}
				 }
